...
def listar_produtos():
    ...
    df.drop(df[df["Preco"] == "0.0"].index, inplace=True)  # ORIGINAL
    df.drop(df[df["Preco"] == "0.00"].index, inplace=True) # ORIGINAL
    df.drop(df[df["Estoque"].isna()].index, inplace=True)  # ORIGINAL
...
def obter_id_por_sku(sku):
    ...
    if response.status_code == 200 and response.json():
        produto = response.json()[0]  # Pega o primeiro resultado
        return produto["id"]  # Retorna o ID do produto
    else:
...
if __name__ == "__main__":
    ...
    #tabela2.to_excel("C:\\TRABALHOS_AUTONOMOS\\ACYR_MONTEIRO\\planilha_agis.xlsx", index=False)
    
    df = listar_produtos()
    tabela_final = df.merge(tabela2, on="SKU", how="inner")  
    #tabela_final.to_excel("C:\\TRABALHOS_AUTONOMOS\\ACYR_MONTEIRO\\planilha_site_ultima.xlsx", index=False)
